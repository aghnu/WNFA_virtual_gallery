...
const assetsURL =
  "https://wnfa-interactive-art-project.github.io/hangzhou_060122/";
...
function loadResults(metaJSON, type) {
  ...
  const createPoster = (url) => {
    ...
    el.oncustomanimationupdate = () => {
      const rotation = Math.abs(
        (GlobalState.getInstance().space_info.rotateDegFocus + r) % 360
      );
      const rotation_corrected =
        Math.floor(rotation / 180) === 0
          ? rotation % 180
          : 180 - (rotation % 180);
      ...
      if (rotation_corrected === 0) {
        ...
        const rotation_corrected_corrected = require_correction
          ? 90 - (rotation_corrected % 90)
          : rotation_corrected;

        const distance2Center =
          z * Math.sin(((90 - rotation_corrected_corrected) * Math.PI) / 180);
    ...
    el.onclick = () => {
      ...
      const site_poster_detail_layer = document.querySelector(
    ...
    preLoadingInterval = setInterval(() => {
      if (canLoad() && next) {
        ...
        getImage(
          assetsURL +
            "results/" +
            Math.floor(Math.random() * postersNum + 1) +
            ".jpg",
          (url) => {
            // success
            if (canLoad()) {
  ...
  const loadPosters = () => {
    ...
    preLoadingInterval = setInterval(() => {
      if (canLoad() && next) {
        ...
        next = false;
        getImage(
          assetsURL + "posters/" + String(i) + ".jpg",
          (url) => {
            // success
            if (canLoad()) {
              appendPoster(createPoster(url));
  ...
  const rotateSpeedUp = (callback = () => {}) => {
    ...
    speedUpAnimationInterval = setInterval(() => {
      if (!pause && GlobalState.getInstance().canRotate() && !pauseForFocus) {
        const target =
          GlobalState.getInstance().rotateSpeed * rotateSpeedUpAnimationSpeed;
  ...
  return {
    init: () => {
      ...
      getMeta((d) => {
        ...
        const gallery_selection = url.searchParams.get("gallery")
        ...
        const music_text_wall_volume = site_wall_text.querySelector(
